 ...
 */
struct ServoManagerConfig {
    ledc_timer_t ledcTimer = LEDC_TIMER_1;            ///< LEDC timer (MUST NOT be TIMER_0, used by DRV8876)
    ledc_timer_bit_t resolution = LEDC_TIMER_14_BIT;  ///< 14-bit for servo precision
...
class ServoManager : public IServoDriver {
  public:
     ...
     */
    ServoManager(const ServoManagerConfig& config, const ServoChannelConfig* channelConfigs, uint8_t numChannels);
     ...
     * @return esp_err_t ESP_OK on success.
     */
    esp_err_t setAngleSmooth(uint8_t servoId, float angleDeg, uint32_t durationMs) override;
  ...
  private:
    ...
    std::array<ServoChannelConfig, MAX_SERVO_CHANNELS> channelConfigs;  ///< Stored configs for init
