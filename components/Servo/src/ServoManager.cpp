...
ServoManager::ServoManager(const ServoManagerConfig& config, const ServoChannelConfig* channelConfigs, uint8_t numChannels) : config(config) {
    channelCount = (numChannels > MAX_SERVO_CHANNELS) ? MAX_SERVO_CHANNELS : numChannels;
...
esp_err_t ServoManager::init() {
    ...
    ESP_LOGI(TAG, "LEDC timer %d configured: %lu Hz, %d-bit resolution", config.ledcTimer, static_cast<unsigned long>(config.frequencyHz),
             static_cast<int>(config.resolution));

    for (uint8_t i = 0; i < channelCount; i++) {
        ret = channels[i].init(channelConfigs[i], config.ledcTimer, config.resolution, config.frequencyHz);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to init servo channel %u: %s", i, esp_err_to_name(ret));
            return ret;
        }
    }
    ...
    taskRunning = true;
    BaseType_t taskRet = xTaskCreate(updateTaskEntry, "servo_mgr", config.taskStackSize, this, config.taskPriority, &updateTaskHandle);
    if (taskRet != pdPASS) {
...
esp_err_t ServoManager::setAngleSmooth(uint8_t servoId, float angleDeg, uint32_t durationMs) {
    if (!isValidServo(servoId)) {
        return ESP_ERR_INVALID_ARG;
    }
...
uint8_t ServoManager::getChannelCount() const { return channelCount; }
...
void ServoManager::updateLoop() {
    ...
    const float deltaTimeSec = static_cast<float>(config.updateIntervalMs) / 1000.0f;
...
bool ServoManager::isValidServo(uint8_t servoId) const { return servoId < channelCount; }
