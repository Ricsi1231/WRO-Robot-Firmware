...
void ServoRamp::startRamp(float fromAngle, float toAngle, float maxSpeedDegPerSec) {
...
void ServoRamp::startTimedRamp(float fromAngle, float toAngle, uint32_t durationMs) {
    targetAngleDeg = toAngle;

    uint32_t safeDuration = (durationMs < MIN_DURATION_MS) ? MIN_DURATION_MS : durationMs;
...
void ServoRamp::cancel() { active = false; }

float ServoRamp::getTarget() const { return targetAngleDeg; }

bool ServoRamp::isActive() const { return active; }
