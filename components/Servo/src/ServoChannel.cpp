...
esp_err_t ServoChannel::init(const ServoChannelConfig& channelConfig,
                             ledc_timer_t timer,
  ...
  if (config.minPulseUs >= config.maxPulseUs) {
    ESP_LOGE(TAG, "Invalid pulse range: min=%.1f >= max=%.1f",
             config.minPulseUs, config.maxPulseUs);
    return ESP_ERR_INVALID_ARG;
  }

  if (config.minAngleDeg >= config.maxAngleDeg) {
    ESP_LOGE(TAG, "Invalid angle range: min=%.1f >= max=%.1f",
             config.minAngleDeg, config.maxAngleDeg);
    return ESP_ERR_INVALID_ARG;
  }
  ...
  ESP_LOGI(TAG, "Initialized on GPIO %d, channel %d, initial angle=%.1f°",
           config.gpioPin, config.ledcChannel, safeAngle);
...
float ServoChannel::getAngle() const {
  return currentAngleDeg;
}

void ServoChannel::setTargetAngle(float angleDeg) {
  targetAngleDeg = clampAngle(angleDeg);
}

float ServoChannel::getTargetAngle() const {
...
bool ServoChannel::isAtTarget() const {
  return std::abs(currentAngleDeg - targetAngleDeg) < ANGLE_TOLERANCE_DEG;
}
...
bool ServoChannel::isAttached() const {
  return attached;
}

ServoChannelConfig ServoChannel::getConfig() const {
  return config;
}
...
uint32_t ServoChannel::angleToPulseTicks(float angleDeg) const {
  ...
  if (config.inverted) {
    effectiveAngle =
        config.maxAngleDeg - angleDeg + config.minAngleDeg;
  }

  float fraction = (effectiveAngle - config.minAngleDeg) /
                   (config.maxAngleDeg - config.minAngleDeg);

  float pulseUs =
      config.minPulseUs + fraction * (config.maxPulseUs - config.minPulseUs);
  ...
  return static_cast<uint32_t>(
      (pulseUs / periodUs) * static_cast<float>(totalTicks) + 0.5f);
}

esp_err_t ServoChannel::writePwm(uint32_t ticks) {
  esp_err_t ret =
      ledc_set_duty(LEDC_LOW_SPEED_MODE, config.ledcChannel, ticks);
  if (ret != ESP_OK) {
