...
AStarSolver::AStarSolver(AStarSolver&& other) noexcept : config(other.config), waypoints(std::move(other.waypoints)), searchPool(std::move(other.searchPool)) {}
...
uint32_t AStarSolver::SearchNode::getScore() const { return g + h; }
...
uint32_t AStarSolver::computeHeuristic(GridPosition source, GridPosition target) const {
...
int32_t AStarSolver::addNode(GridPosition position) {
    ...
    ESP_LOGI(TAG, "Added node %ld at (%ld,%ld)", static_cast<long>(index), static_cast<long>(position.x), static_cast<long>(position.y));
...
esp_err_t AStarSolver::addEdge(int32_t from, int32_t to) {
    if (!isValidNode(from) || !isValidNode(to)) {
        ESP_LOGE(TAG, "Invalid edge indices: from=%ld, to=%ld (nodeCount=%zu)", static_cast<long>(from), static_cast<long>(to), waypoints.size());
    ...
    uint32_t weight = computeEdgeWeight(waypoints[from].position, waypoints[to].position);
...
esp_err_t AStarSolver::findPath(int32_t startNode, int32_t targetNode, Path& outPath) {
    outPath.clear();

    if (!isValidNode(startNode) || !isValidNode(targetNode)) {
        ESP_LOGE(TAG, "Invalid node indices: start=%ld, target=%ld (nodeCount=%zu)", static_cast<long>(startNode), static_cast<long>(targetNode),
                 waypoints.size());
        return ESP_ERR_INVALID_ARG;
    }
    ...
    startSearch.g = 0;
    startSearch.h = computeHeuristic(waypoints[startNode].position, waypoints[targetNode].position);
    ...
    while (!openList.empty()) {
        ...
        if (currentWpIdx == targetNode) {
            ...
            ESP_LOGI(TAG, "Path found from node %ld to %ld, length=%u", static_cast<long>(startNode), static_cast<long>(targetNode),
        ...
        for (const auto& edge : waypoints[currentWpIdx].edges) {
            ...
            if (existingIdx != NO_PARENT) {
                ...
                neighborSearch.h = computeHeuristic(waypoints[neighborWpIdx].position, waypoints[targetNode].position);
    ...
    ESP_LOGW(TAG, "No path found from node %ld to %ld", static_cast<long>(startNode), static_cast<long>(targetNode));
    return ESP_ERR_NOT_FOUND;
}

void AStarSolver::setHeuristic(HeuristicType type) { config.heuristicType = type; }

AStarConfig AStarSolver::getConfig() const { return config; }
