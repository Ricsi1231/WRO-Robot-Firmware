...
class GpioInterruptManager : public IGpioInterruptManager {
  public:
     ...
     * @return esp_err_t ESP_OK on success.
     */
    esp_err_t registerInterrupt(gpio_num_t pin, EdgeType edge, GpioPull pull, std::function<void(gpio_num_t)> callback) override;
  ...
  private:
    ...
    /// Timeout for dispatch task queue receive (ms)
    static constexpr uint32_t DISPATCH_TIMEOUT_MS = 50;
